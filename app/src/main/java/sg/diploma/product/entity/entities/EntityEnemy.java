...
public final class EntityEnemy extends EntityAbstract{
	private EntityEnemy(final int bitmapID){
		super();
		attribs.renderLayer = EntityRenderLayers.EntityRenderLayer.Normal;
		attribs.type = EntityTypes.EntityType.Enemy;
	...
	public void Update(float dt){
		...
		if(attribs.pos.y - attribs.scale.y * 0.5f >= EntityManager.Instance.cam.GetPos().y + DeviceManager.screenHeightF){
			EntityManager.Instance.SendEntityForRemoval("Enemy_" + myIndex);
		}
		//*/
	}
	...
	public static EntityEnemy Create(final String key, final int bitmapID){
		EntityEnemy result = new EntityEnemy(bitmapID);
