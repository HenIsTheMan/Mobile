...
public final class EntityGamePlayerChar extends EntityAbstract{
	...
	public void Update(final float dt){
		...
		if(attribs.colliderPos.y + attribs.colliderScale.y * 0.5f >= EntityManager.Instance.cam.GetPos().y + DeviceManager.screenHeightF
			|| attribs.colliderPos.y - attribs.colliderScale.y * 0.5f <= EntityManager.Instance.cam.GetPos().y){
	...
	public void Collided(EntityAbstract other){
		if(other.attribs.type == EntityTypes.EntityType.Plat && currPlat == null && attribs.vel.y >= 0.0f){
			...
			attribs.colliderPos.y = other.attribs.colliderPos.y - (other.attribs.colliderScale.y + attribs.colliderScale.y) * 0.5f;
			attribs.pos.y = attribs.colliderPos.y - attribs.colliderScale.y * 0.075f;
			return;
		}

		if(other.attribs.type == EntityTypes.EntityType.Enemy){
			Publisher.Broadcast(new EventEndGame());
			return;
		}
	}
